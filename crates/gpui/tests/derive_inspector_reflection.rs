...
fn derives_inspector_reflection() {
    ...
    let result = find_method::<Number>("double")
        .map(|method: gpui::inspector_reflection::FunctionReflection<Number>| method.invoke(number))
        .and_then(|number| {
            find_method::<Number>("increment").map(
                |method: gpui::inspector_reflection::FunctionReflection<Number>| {
                    method.invoke(number)
                },
            )
        })
        .and_then(|number| {
            find_method::<Number>("triple").map(
                |method: gpui::inspector_reflection::FunctionReflection<Number>| {
                    method.invoke(number)
                },
            )
