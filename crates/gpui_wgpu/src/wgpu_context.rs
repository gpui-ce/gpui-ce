...
impl WgpuContextHandle {
    pub(crate) fn from_resources(
        ...
        let backend = WgpuBackend::Native(adapter_info.backend);
        Self {
            device,
            queue,
            texture_format,
    ...
    ///
    /// A control can use this to detect device replacement when recovery
    /// happens between frames and the transient device-lost state was not
    /// observed.
...
///
/// The target owns only a texture and view. Pipelines, buffers, and command
/// encoders remain under the control of the custom element.
