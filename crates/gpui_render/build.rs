...
//! (WebGpu WGSL, downlevel WGSL for WebGL2/GLES, native HLSL/MSL) and validates each
...
fn decode_expr(
    ...
    match &module.types[handle].inner {
        ...
        naga::TypeInner::Array { base, size, .. } => {
            let element_stride = layouter[handle].to_stride().div_ceil(4);
...
// --- Native artifacts: HLSL via floor Naga, MSL via current Naga ---------------------
...
fn write_native_shaders(out_dir: &std::path::Path) {
    ...
    let mut validated_labels: BTreeSet<&str> = BTreeSet::new();
    for module in modules {
        ...
        if validated_labels.insert(pipeline.label) {
            validate_artifact(&ShaderArtifact::modern(pipeline.label, wgsl.clone()));
        }
